    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
        <form onSubmit={saveProfile} className="card space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><label className="form-label">First Name</label><input required minLength={2} maxLength={60} className="form-input" value={form.firstName} onChange={(event) => update('firstName', event.target.value)} /></div><div><label className="form-label">Last Name</label><input required minLength={2} maxLength={60} className="form-input" value={form.lastName} onChange={(event) => update('lastName', event.target.value)} /></div><div><label className="form-label">Email</label><input required type="email" maxLength={254} className="form-input" value={form.email} onChange={(event) => update('email', event.target.value)} /></div><div><label className="form-label">Position</label><input maxLength={100} className="form-input" value={form.position} onChange={(event) => update('position', event.target.value)} /></div></div>
