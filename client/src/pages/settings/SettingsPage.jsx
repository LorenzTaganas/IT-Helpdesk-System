...
const SettingsPage = () => {
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><div className="page-header"><div><h1 className="page-title flex items-center gap-2.5"><SettingsIcon className="text-blue-600" size={26} /> Settings</h1><p className="page-subtitle">Configure shared helpdesk defaults and service expectations.</p></div></div><form onSubmit={save} className="space-y-6"><section className="card space-y-5"><div><h2 className="text-base font-bold text-slate-900">Helpdesk Configuration</h2><p className="mt-1 text-sm text-slate-500">These values apply across the ITFlow workspace.</p></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><label className="form-label">Organization Name</label><input required minLength={2} maxLength={120} className="form-input" value={form.organizationName} onChange={(event) => update('organizationName', event.target.value)} /></div><div><label className="form-label">Support Email</label><input type="email" maxLength={254} className="form-input" value={form.supportEmail} onChange={(event) => update('supportEmail', event.target.value)} placeholder="support@example.com" /></div><div><label className="form-label">Timezone</label><select required className="form-select" value={form.timezone} onChange={(event) => update('timezone', event.target.value)}><option>UTC</option><option>Asia/Manila</option><option>America/New_York</option><option>America/Los_Angeles</option><option>Europe/London</option></select></div><div><label className="form-label">Default Ticket Priority</label><select required className="form-select" value={form.defaultTicketPriority} onChange={(event) => update('defaultTicketPriority', event.target.value)}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></div><div><label className="form-label">SLA Target (days)</label><input required type="number" min="1" max="365" className="form-input" value={form.slaDays} onChange={(event) => update('slaDays', event.target.value)} /></div></div></section><section className="card"><label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={form.allowEmployeeClose} onChange={(event) => update('allowEmployeeClose', event.target.checked)} className="mt-1 rounded border-slate-300 text-blue-600 focus:ring-blue-500" /><span><strong className="block text-sm text-slate-900">Allow employees to close their own tickets</strong><small className="mt-1 block text-sm text-slate-500">Employees can mark their own resolved requests as closed.</small></span></label></section><div className="flex justify-end"><button disabled={saving} className="btn btn-primary"><Save size={16} /> {saving ? 'Saving...' : 'Save Settings'}</button></div></form></div>;
};
